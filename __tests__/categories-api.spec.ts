...
jest.mock("@/lib/prisma", () => ({
  prisma: {
    category: { findMany: jest.fn(), findFirst: jest.fn(), findUnique: jest.fn(), create: jest.fn(), update: jest.fn(), delete: jest.fn() },
  },
}));
...
describe("GET /api/categories", () => {
  ...
  it("retorna summary com filtro", async () => {
    mockWithAuth.mockResolvedValue({ id: "u1", email: "a@b.com" });
    mockSummary.mockResolvedValue([{ id: "c1", name: "Alimentação", color: "#f00", value: 200, number: 2, relationship: ["Mercado"] }]);
    const res = await LIST_GET(new Request("http://localhost/api/categories?type=summary&month=6&year=2024"));
    expect(res.status).toBe(200);
